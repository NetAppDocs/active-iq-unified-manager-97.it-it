...
====
Per ulteriori informazioni sulle soglie di performance dinamiche, vedere link:reference-performance-event-analysis-and-notification.html["Quali sono gli eventi"]. Per informazioni su come Unified Manager classifica i carichi di lavoro e determina l'ordinamento, vedere xref:concept-how-unified-manager-determines-the-performance-impact-for-an-incident.adoc[In che modo Unified Manager determina l'impatto delle performance di un evento].
...
+
Visualizza i dettagli sull'utilizzo del carico di lavoro del componente del cluster in conflitto. Nei grafici, l'utilizzo effettivo è una linea blu. Una barra rossa evidenzia la durata dell'evento, dal tempo di rilevamento all'ultimo tempo analizzato. Per ulteriori informazioni, vedere link:reference-workload-performance-measurement-values.html["Misurazioni delle performance dei carichi di lavoro"].
