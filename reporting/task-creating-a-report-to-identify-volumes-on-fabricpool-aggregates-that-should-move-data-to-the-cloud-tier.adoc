...
. Nel selettore di colonne, assicurarsi che la colonna "`Disk Type`" (tipo di disco) venga visualizzata nella vista.
+
...
In base ai risultati mostrati nel report, è possibile utilizzare Gestione di sistema o l'interfaccia CLI di ONTAP per modificare la policy di tiering in "`auto`" o "`all`" per alcuni volumi per trasferire più dati cold al livello cloud.
